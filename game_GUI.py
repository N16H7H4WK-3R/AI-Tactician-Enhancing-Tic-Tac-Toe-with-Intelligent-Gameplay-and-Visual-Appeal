...
class Game:
    def __init__(self, root):
        ...
        self.make_canvas = Canvas(
            self.window, background="#d9d9d9", relief=RAISED, bd=3)
    ...
    def decoration(self):
        # Adds decorative labels and buttons to the game window.

        # Adding labels
        Label(self.make_canvas, text="A.I. Tactician", bg="#d9d9d9",
              fg="black", font=("Lato", 13, "bold")).place(x=180, y=10)
        Label(self.make_canvas, text="Enhanced with Intelligent Gameplay", bg="#d9d9d9",
              fg="black", font=("Lato", 15)).place(x=70, y=30)

        buttons = []
        coordinates = [(35, 100), (190, 100), (335, 100), (35, 200),
                       (190, 200), (335, 200), (35, 300), (190, 300), (335, 300)]

        # Adding buttons for the Tic-Tac-Toe board
        for i in range(1, 10):
            btn = Button(self.make_canvas, text="", font=("Arial", 15, "bold"), width=5, bg="#fff",
                         activebackground="black", bd=0, command=lambda num=i: self.__user_turn(num), state=DISABLED,
                         relief="solid", borderwidth=3)
            btn.place(x=coordinates[i - 1][0], y=coordinates[i - 1][1])
            buttons.append(btn)

        self.activate_btn = buttons

        # Adding control buttons
        self.machine_first_control = Button(self.make_canvas, text="AI vs YOU", font=("Arial", 15, "bold"), bg="#fff",
                                            activebackground="black", fg="black", relief=SOLID, bd=3, command=lambda: self.control_give("machine_first"), pady=5)
        self.machine_first_control.place(x=35, y=430)

        self.human_first_control = Button(self.make_canvas, text="YOU vs AI", font=(
            "Arial", 15, "bold"), bg="#fff", activebackground="black", fg="black", relief=SOLID, bd=3, command=self.control_give, pady=5)
        self.human_first_control.place(x=293, y=430)

        self.reset_btn = Button(self.make_canvas, text=" Reset ", font=("Arial", 15, "bold"), width=30, height=1, bg="#fff",
                                activebackground="black", disabledforeground="blue", fg="black", relief=SOLID, bd=3, command=self.reset, state=DISABLED, pady=5)
        self.reset_btn.place(x=35, y=370)
    ...
    def game_over(self):
        # Disables all buttons and enables the reset button when the game is over.
        for every in self.activate_btn:
            every.config(state=DISABLED)
        self.reset_btn['state'] = NORMAL

    def control_give(self, indicator="human_first"):
        # Disables or enables control buttons based on the game mode.
    ...
    def __sign_insert(self, btn_indicator, sign_is="X"):
        # Inserts a sign (X or O) at the specified button position.
        if sign_is == "X":
            self.activate_btn[btn_indicator - 1].config(
                text=sign_is, state=DISABLED, disabledforeground="#276221")
        else:
            self.activate_btn[btn_indicator - 1].config(
    ...
    def __ai_turn(self):
        # Performs the AI's turn in the game.

        # Increase the chance counter
        self.chance_counter += 1

        # Execute different strategies based on the chance counter
        if self.chance_counter == 1:
            ...
            if self.technique == 1:
                ...
                # If Technique 2 was used previously
    ...
    def ai_match(self):
        ...
        # If the AI wins, displays a message indicating the winner and ends the game.
        found = 0
        # Check all possible winning combinations for the AI (X)
        # If any combination is found, set found to 1
        if self.activate_btn[1-1]['text'] == self.activate_btn[2-1]['text'] == self.activate_btn[3-1]['text'] == "X":
            ...
            found = 1
        if found == 1:
            # Display a message indicating the winner (AI) and end the game
            messagebox.showinfo("Game Over", "Computer is winner")
            self.game_over()
        elif self.chance_counter == 9:
            # If no winner and the board is full, the game is a draw
            messagebox.showinfo("Game Over", "Game draw")
            self.game_over()

    def user_match(self):
        # Checks if the user has won the game.
        # If the user wins, displays a message indicating the winner and ends the game.
        found = 0
        # Check all possible winning combinations for the user (O)
        # If any combination is found, set found to 1
        if self.activate_btn[1-1]['text'] == self.activate_btn[2-1]['text'] == self.activate_btn[3-1]['text'] == "O":
            ...
            found = 1
        if found == 1:
            # Display a message indicating the winner (user) and end the game
            messagebox.showinfo("Game Over", "Congrats! You are the winner")
            self.game_over()
            return 0
        elif self.chance_counter == 9:
            # If no winner and the board is full, the game is a draw
...
if __name__ == "__main__":
    ...
    window.config(bg="#141414")  # Configure the background color of the window
    # Set the initial size and limits of the window
    window.geometry("450x500")
    window.maxsize(450, 500)
    window.minsize(450, 500)
    # Create an instance of the Game class, passing the window object as a parameter
    Game(window)
